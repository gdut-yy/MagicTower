...
import com.mymt.util.ImageUtil;
...
//                    exp += e.getExp();
//                    money += e.getMoney();
//                    gameUtil.displayMessage("获得金币数 " + e.getExp() + " 经验值 " + e.getMoney() + " ！");
...
//    }
    ...
    public BufferedImage draw() {
        if (toward == 0)
            return ImageUtil.playerMap.get(-1);
        if (toward == 1)
            return ImageUtil.playerMap.get(-2);
        if (toward == 2)
            return ImageUtil.playerMap.get(-3);
        if (toward == 3)
            return ImageUtil.playerMap.get(-4);
        return null;
    }
